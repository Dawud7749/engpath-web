    ...
    <OnboardingShell step={4} total={5} title="Mau fokus skill apa?" subtitle="Kamu tetap belajar semua, tapi yang ini dapat porsi lebih.">
      <form action={saveOnboardingStep} style={{ display: 'flex', flexDirection: 'column', flex: 1 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          {focuses.map((f, i) => (
            <label key={f.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, padding: '20px 12px', background: '#fff', borderRadius: 'var(--r-md)', border: '2px solid rgba(26,43,60,0.08)', cursor: 'pointer', position: 'relative', textAlign: 'center' }}>
